...
public:
    ...
    // 0/null on error
    UmlPackage * importProject(const WrapperStr & fn);
    ...
    const WrapperStr & phpNamespace();

    // the namespace name associed to the package.
    //
    // On error : return FALSE in C++, produce a RuntimeException in Java
    bool set_PhpNamespace(const WrapperStr & v);

    // returns a sub package of the current one having the php namespace 'n'
    // (including the current one), else 0/null

    UmlPackage * findPhpNamespace(const WrapperStr & n);
#endif
