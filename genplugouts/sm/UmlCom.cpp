...
void UmlCom::read_buffer(size_t len)
{
    ...
    for (;;) {
        if ((nread = sock->read(p, remainder)) == -1) {
            ...
            {
#ifdef TRACE
                qDebug() << "UmlCom::read_buffer ERROR, already x" << p - buffer_in
                         << " remainder " << remainder << '\n';
#endif
                fatal_error("UmlCom read errorx");
                nread = 0;
            }
        }
...
{
#ifdef TRACE
    qDebug() << "UmlCom::send_cmd(id, " << cmd << ", const QVector<UmlClass*> & l1, const QVector<UmlClass*> & l2, const QVector<UmlClass*> & l3)\n";
#endif
